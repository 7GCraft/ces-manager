...
const handle = () => {
  ...
  ipcMain.on(
    ...
    getUsedResourceComponentListByState
  );
  ipcMain.on(
    "Component:getMultipleUsedResourceComponentListByState",
    getMultipleUsedResourceComponentListByState
  );
    ...
    .then((result) => {
      if (result != null) {
        return Promise.all(
          result.map((facility) => {
            return component
              .getComponentByFacilityId(facility.facilityId)
              .then((components) => {
                return component
                  .sortChildComponents(components)
...
const openBulkInsertPage = (e, arg) => {
  ...
  bulkInsertComponentsWindow.loadFile(
    "src/views/region/component/bulkInsert.html"
  );
