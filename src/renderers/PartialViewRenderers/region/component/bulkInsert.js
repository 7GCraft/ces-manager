...
function frmAddComponent_SubmitHandler(e) {
  ...
  if (validateAddComponent(data)) {
    ...
    if (data.isChild) {
      ...
      let showChildren = parseInt(
        $(showChildrenbtn).attr("data-show-children")
      );
      if (showChildren === 0) {
        let childrenRowCount = $(`.${parentRowId}-child`).length;
        updateShowChildrenButtonTooltip(
          showChildrenbtn,
          `Show Children (${childrenRowCount})`
        );
      }
    } else {
...
function updateUnusedPopulation(data, isDelete) {
  if (data.componentType.componentTypeId == 1) {
    ...
    if (isDelete) usedPopulation = usedPopulation * -1;
...
function createRowFromData(data) {
  ...
  const colorClass = getComponentTypeColorClass(
    data.componentType.componentTypeId
  );

  let typeCell = clonedTemplateRow.find("#typeCell");
  let componentTypeValue = ComponentTypes.find(
    findComponentTypeByID,
    data.componentType.componentTypeId
  );
  typeCell.text(
    componentTypeValue === undefined ? "" : componentTypeValue.componentTypeName
  );
  typeCell.addClass(colorClass);

  let valueText = data.value;
  if (data.componentType.componentTypeId == 3) {
    let resourceValue = Resources.find(
      findResourceByID,
      parseInt(data.value.resourceId)
    );
  ...
  clonedTemplateRow
    .find("#facilityCell")
    .text(facilityValue === undefined ? "NONE" : facilityValue.facilityName);
...
function createChildrenRow(rowTemplate, parentRowId) {
  ...
  let showChildren = parentRow
    .find("#actionCell")
    .find(".btnShowChildren")
    .attr("data-show-children");
...
function btnShowChildren_ClickHandler(btn) {
  ...
  let tooltipTitle =
    showChildren === 1
      ? "Hide children"
      : `Show children (${childrenRow.length})`;
...
function showSummaryModal() {
  ...
  if (data.length === 0) {
    if (
      alertWrapper
        .children(".alert")
        .not("#emptyTableTemplateAlert, #insertResultTemplateAlert").length >= 1
    ) {
      return;
    }

    let emptyTableAlert = $("#emptyTableTemplateAlert")
      .clone()
      .removeAttr("id");
  ...
  ComponentTypes.forEach((componentType) => {
    $(`#summary${componentType.componentTypeName}`).text(
      summary.ComponentType[i]
    );
    i++;
  });
...
function doBulkInsert() {
  ...
  ipcRenderer.send(
    "Component:addMultipleComponents",
