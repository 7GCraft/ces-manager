...
function replaceElementAttributeContent(
  element,
  attribute,
  stringToReplace,
  newString
  ...
  $(element).attr(
    attribute,
    attributeContent.replace(stringToReplace, newString)
  );
}
...
function getComponentsInfo() {
  ...
  ipcRenderer.once("Component:getComponentListOK", (e, res) => {
    ...
    const parentChildComponentsDict =
      mapParentWithChildComponent(childComponents);
    localStorage.setItem(
      "childComponents",
      JSON.stringify(parentChildComponentsDict)
    );
...
function filterComponents() {
  let componentUsabilityFilterMode = $(
    "input[name=usedComponentDisplay]:checked"
  ).val();
  let componentActivationFilterMode = $(
    "input[name=activatedComponentDisplay]:checked"
  ).val();
  ...
  if (componentUsabilityFilterMode === USED) {
    componentList = componentList.filter(
      (component) => component.facilityId !== null
    );
  } else if (componentUsabilityFilterMode === UNUSED) {
    componentList = componentList.filter(
      (component) => component.facilityId === null
    );
  }
  ...
  if (componentActivationFilterMode === ACTIVATED) {
    componentList = componentList.filter(
      (component) => component.activationTime === 0
    );
  } else if (componentActivationFilterMode === UNACTIVATED) {
    componentList = componentList.filter(
      (component) => component.activationTime !== 0
    );
  }
...
function addUpdateComponent_handler() {
  ...
  $("#frmAddUpdateComponent").on("submit", (e) => {
    ...
    componentObj["facilityId"] =
      $("#selFacility").val() == "" ? null : $("#selFacility").val();
    ...
    if (componentTypeId == 3) {
      ...
      componentObj["value"] =
        componentTypeId == 1 || componentTypeId == 4 || componentTypeId == 5
          ? parseInt($("#txtValue").val() || 0)
          : $("#txtValue").val();
    }
    ...
    if (!validate_addUpdateComponent(componentObj, componentTypeId)) return;
    ...
    if ($("#hdnComponentId").val() == "") {
      ...
      ipcRenderer.once("Component:updateComponentOK", (e, res) => {
        if (res) {
          ...
          $("#regionMessage").append(
            '<div class="alert alert-danger alert-dismissible"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>Something went wrong when updating component</div>'
          );
        }
...
function validate_addUpdateComponent(componentObj, componentType) {
  ...
  if (
    ...
    $("#lblValueErrMessage").text(
      "Value must not be bigger than unused population"
    );
  } else {
...
function deleteComponent_handler() {
  $("#btnConfirmDeleteComponent").on("click", (e) => {
    e.preventDefault();
    let componentId = $("#btnConfirmDeleteComponent")
      .data("componentId")
      .replace("Component", "");
    ...
    ipcRenderer.once("Component:deleteComponentOK", (e, res) => {
      if (res) {
        ...
        $("#stateMessage").append(
          '<div class="alert alert-danger alert-dismissible"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>Something went wrong when deleting component</div>'
        );
      }
...
function setComponentList(res) {
  ...
  if (Array.isArray(res) && res.length) {
    ...
    indexComponentTable("componentsList", "numberCell", [
...
function createComponentRow(component) {
  ...
  let classType = getComponentTypeColorClass(
    component.componentType.componentTypeId
  );
  let templateRowId = component.isChild
    ? "#childComponentTemplateRow"
    : "#componentTemplateRow";
  let clonedTemplate = $(templateRowId)
    .clone()
    .attr("id", rowId)
  ...
  let valueText =
    component.componentType.componentTypeId == 3
      ? component.value.ResourceName
      : component.componentType.componentTypeId == 2
  ...
  clonedTemplate
    .find("#typeCell")
    .text(component.componentType.componentTypeName)
    .addClass(classType);
  ...
  if (!component.isChild) {
    ...
    replaceElementAttributeContent(
      showChildBtn,
      "onclick",
      "{componentId}",
  ...
  let updateComponentBtn = clonedTemplate
    .find("#actionCell")
    .find("#btnUpdateComponent");
  replaceElementAttributeContent(
    updateComponentBtn,
    "onclick",
    "{rowId}",
  ...
  let deleteComponentBtn = clonedTemplate
    .find("#actionCell")
    .find("#btnDeleteComponent");
  replaceElementAttributeContent(
    deleteComponentBtn,
    "onclick",
    "{rowId}",
...
function showChildComponents(parentId) {
  ...
  indexComponentTable("childComponentsList", "numberCell", [
    "#childComponentTemplateRow",
  ]);
}
...
function mdlChildComponents_eventHandler() {
  $("#mdlChildComponents").on(
    "shown.bs.modal",
    mdlChildComponents_shownEventHandler
  );
  $("#mdlChildComponents").on(
    "hidden.bs.modal",
    mdlChildComponents_hiddenEventHandler
  );
}
