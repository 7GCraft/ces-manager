...
function getFacilitiesInfo() {
  let facilityIds = [];
  ipcRenderer.send(
    "Facility:getFacilitiesByRegion",
    parseInt(window.process.argv.slice(-1))
  );
  ipcRenderer.once("Facility:getFacilitiesByRegionOK", (e, res) => {
    ...
    if (res != null) {
      $("#facilityList").empty();
      let facilityFilter = $(
        "input[name=functionalFacilitiesDisplay]:checked"
      ).val();
      ...
      res.forEach((facility) => {
        let foodOutput =
          facility.foodOutput == 0
            ? ""
            : '<span class="valueFood">Food Output: ' +
              facility.foodOutput +
              "</span><br/>";
        let moneyOutput =
          facility.moneyOutput == 0
            ? ""
            : '<span class="valueMoney">Money Output: ' +
              facility.moneyOutput +
              "</span><br/>";
        let resource =
          facility.resource == null
            ? ""
            : '<span class="valueResource">Resource: ' +
              facility.resource.ResourceName +
              "</span><br/>";

        let noOutput =
          foodOutput == "" && moneyOutput == "" && resource == ""
            ? "No Output for this facility<br/>"
            : "";
        $("#facilityList").append(
          '<div class="card">' +
            '<div class="card-header" id="FacilityHeading' +
            facility.facilityId +
            '">' +
            ' <h2 class="mb-0">' +
            '<button id="btnFacility' +
            facility.facilityId +
            '" class="btn btn-link btnFacilityInfo collapsed" type="button" data-toggle="collapse" data-target="#FacilityCollapse' +
            facility.facilityId +
            '"" aria-expanded="false" aria-controls="FacilityCollapse' +
            facility.facilityId +
            '">' +
            facility.facilityName +
            "</button>&nbsp;" +
            '<input type="textbox" id="txtUpdateFacility' +
            facility.facilityId +
            '" data-facility-id="' +
            facility.facilityId +
            '" data-functional="' +
            facility.isFunctional +
            '" style="height: 25px; width:200px; font-size:16px;" onkeyup="if(event.keyCode === 13){updateFacilityName(this.id);}" >&nbsp;&nbsp;' +
            '<input type="image" id="imgUpdateFacility' +
            facility.facilityId +
            '" src="../images/icons/edit.png" style="height: 15px; width:15px;" onclick=toggleUpdateFacilityNameTextbox(this.id)>' +
            "</h2>" +
            "</div>" +
            '<div id="FacilityCollapse' +
            facility.facilityId +
            '" class="collapse" aria-labelledby="FacilityHeading' +
            facility.facilityId +
            '" data-parent="#facilityList">' +
            '<div class="card-body">' +
            '<div class="row">' +
            '<div class="column" id="Facility' +
            facility.facilityId +
            '">' +
            foodOutput +
            moneyOutput +
            resource +
            noOutput +
            'Functional:   <input type="checkbox" id="chkFunctional' +
            facility.facilityId +
            '" data-facility-name = "' +
            facility.facilityName +
            '" onclick=updateFunctional(this.id)><br/><br/>' +
            ...
            "</div>" +
            '<div class="column"><ul  id="FacilityComponents' +
            facility.facilityId +
            '"></ul></div>' +
            "</div>" +
            "</div>" +
        ...
        if (facility.isFunctional) {
          $("#FacilityHeading" + facility.facilityId).css(
            "background-color",
            "#c9f0f2"
          );
  ...
  ipcRenderer.send(
    "Component:getComponentByFacilityId",
    parseInt(window.process.argv.slice(-1))
  );
      ...
      res.forEach((components) => {
        if (components != null) {
          components.forEach((component) => {
            let facilityId = facilityIds.find(
              (id) => id == component.facilityId
            );

            if (!component.isChild) {
              $("#FacilityComponents" + facilityId).append(
                '<li id="ComponentFacility' +
              ...
              $("#ComponentFacility" + component.parentId).append(
                '<ul><li id="ComponentFacility' +
                  component.componentId +
                  '"><b>' +
                  component.componentName +
                  "</b></li></ul>"
  ...
  $("#frmAddFacility").on("submit", (e) => {
    ...
    ipcRenderer.once("Facility:addFacilityOK", (e, res) => {
      if (res) {
        ...
        $("#regionMessage").append(
          '<div class="alert alert-danger alert-dismissible"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>Something went wrong when adding facility</div>'
        );
      }
    });
  });
...
function updateFacilityName(txtUpdateFacilityNameId) {
  ...
  ipcRenderer.once("Facility:updateFacilityOK", (e, res) => {
    if (res) {
      ...
      $("#regionMessage").append(
        '<div class="alert alert-danger alert-dismissible"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>Something went wrong when updating facility</div>'
      );
    }
  });
}
...
function addDeleteListener() {
  $("#btnDeleteFacility").on("click", (e) => {
    e.preventDefault();
    let facilityId = $("#btnDeleteFacility")
      .data("facilityId")
      .replace("Facility", "");
    ...
    ipcRenderer.once("Facility:deleteFacilityOK", (e, res) => {
      if (res) {
        ...
        $("#stateMessage").append(
          '<div class="alert alert-danger alert-dismissible"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>Something went wrong when deleting facility</div>'
        );
      }
