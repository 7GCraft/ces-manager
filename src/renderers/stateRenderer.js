...
function getStateInfo() {
  ...
  ipcRenderer.once("State:getStateInfoOK", function (e, res) {
    ...
    $("#lblTotalExpenses").text(
      parseFloat(res.expenses + res.adminCost).toFixed(2)
    );
    $("#lblNextIncome").text(
      parseFloat(res.TotalIncome - res.expenses - res.adminCost).toFixed(2)
    );
...
function getResources() {
  ipcRenderer.send(
    "Resource:getAllResourcesByStateId",
  ...
  ipcRenderer.once("Resource:getAllResourcesByStateIdOk", (e, res) => {
    ...
    if (Array.isArray(res) && res.length) {
      ...
      indexTable("tblResources", "numberCell", [
        "#resourceTemplateRow",
        "#noResourceRow",
      ]);
    } else {
...
function getTradeAgreements() {
  ipcRenderer.send(
    "Trade:getTradeAgreementsByStateId",
    parseInt(getProcessArgObj())
  );
  ...
  $("#btnDeleteState").on("click", (e) => {
    ...
    ipcRenderer.once("State:deleteStateOK", (e, res) => {
      if (res) {
        ...
        $("#stateMessage").append(
          '<div class="alert alert-danger alert-dismissible"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>Something went wrong when deleting state</div>'
        );
      }
...
function frmUpdateState_onSubmit() {
  $("#frmUpdateState").on("submit", (e) => {
    ...
    stateObj["adminRegionModifier"] =
      $("#nmbAdminRegionModifier").val() == ""
        ? 0
        : parseFloat($("#nmbAdminRegionModifier").val() / 100);
    stateObj["expenses"] =
    ...
    ipcRenderer.once("State:updateStateOK", (e, res) => {
      if (res) {
        ...
        $("#stateMessage").append(
          '<div class="alert alert-danger alert-dismissible"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>Something went wrong when updating state</div>'
        );
      }
