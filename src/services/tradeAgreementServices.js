...
const getTradeAgreementAll = async (returnsList = true) => {
  ...
  if (
    rawTradeAgreementHeaders.length === 0 ||
    rawTradeAgreementDetails.length === 0
  )
    return null;
  ...
  for (let rawTradeAgreementDetail of rawTradeAgreementDetails) {
    ...
    if (rawTradeAgreementDetail.resourceComponentId !== null) {
      ...
      if (
        componentDict[rawTradeAgreementDetail.resourceComponentId] === undefined
      ) {
        ...
        componentDict[
          rawTradeAgreementDetail.resourceComponentId
        ].tradeAgreementIds.push(rawTradeAgreementDetail.tradeAgreementId);
        // add the resource component ID to the component IDs list
        componentIds.push(rawTradeAgreementDetail.resourceComponentId);
      } else if (
        componentDict[rawTradeAgreementDetail.resourceComponentId].stateId ===
        rawTradeAgreementDetail.stateId
      ) {
        componentDict[
          rawTradeAgreementDetail.resourceComponentId
        ].tradeAgreementIds.push(rawTradeAgreementDetail.tradeAgreementId);
  ...
  let components = await componentServices.getComponentFunctionalByIds(
    componentIds
  );

  let resourceTiers = await resourceServices.getResourceTierAll();

  if (states === null || components === null || resourceTiers === null)
    return null;
  ...
  for (let key of Object.keys(componentDict)) {
    ...
    for (let tradeAgreementId of component.tradeAgreementIds) {
      stateDict[component.stateId.toString()].tradeAgreements[
        tradeAgreementId
      ].push(component.resource);
    }
  }

  for (let nullComponent of nullComponents) {
    stateDict[nullComponent.stateId.toString()].tradeAgreements[
      nullComponent.tradeAgreementId
    ] = null;
  }
  ...
  for (let stateKey of Object.keys(stateDict)) {
    for (let key of Object.keys(stateDict[stateKey].tradeAgreements)) {
      let trader = new Trader(stateDict[stateKey].state, {
        resources: stateDict[stateKey].tradeAgreements[key],
      });
