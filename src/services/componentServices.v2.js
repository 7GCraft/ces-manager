...
const addComponent = async ({ component, tempFacilityName = null }) => {
  ...
  const newValue = convertComponentInputValue(component.value);
  ...
  const isTreasurySufficient = await calculateComponentCost(component.cost, component.regionId);
  ...
  if (tempFacilityName != null) {
    ...
    const tempFacilityMap = new Map([[mapKey, tempFacilityName]]);
    const facilityInsertStatus = await allocateTemporaryFacilitiesToComponents([component], tempFacilityMap);
...
const addMultipleComponents = async ({ components, tempFacilityMap = null }) => {
  ...
  components.forEach((component) => {
    const tempValue = convertComponentInputValue(component.value);
  ...
  const isTreasurySufficient = await calculateComponentCost(totalCost, components[0].regionId);
  ...
  try {
    if (tempFacilityMap != null) {
      const facilityInsertStatus = await allocateTemporaryFacilitiesToComponents(parentArray, tempFacilityMap);
      if (!facilityInsertStatus) {
...
const allocateTemporaryFacilitiesToComponents = async (components, tempFacilityMap) => {
  const { regionId } = components[0];
  const insertedFacilityMap = await processTemporaryFacilities(tempFacilityMap, regionId);
...
const processTemporaryFacilities = async (tempFacilityMap, regionId) => {
  ...
  const facilityInsertStatus = await facilityServices.addMultipleFacilities(temporaryFacilities);
  if (!facilityInsertStatus) {
    return null;
  }
