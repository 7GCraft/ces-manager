...
const getComponentByRegionId = async (id) => {
  ...
  for (let rawComponent of rawComponents) {
    ...
    if (rawComponent.componentTypeId === 3) {
      componentValue =
        resources[parseInt(rawComponent.value.split(";")[1]) - 1];
    }
...
const getComponentFunctionalByRegionId = async (id) => {
  const rawComponents = await knex
    ...
    .andWhere(
      constants.TABLE_FACILITY + "." + constants.COLUMN_IS_FUNCTIONAL,
      1
    )
    .catch((e) => {
      console.error(e);
...
const addComponent = async (component) => {
  ...
  if (component.value !== null) {
    if (typeof component.value === "number") newValue = `i;${component.value}`;
      ...
      newValue = `s;${component.value}`;
    else if (typeof component.value === "object")
      newValue = `i;${component.value.resourceId}`;
  }
  ...
  let costCalcReturn = await calculateComponentCost(
    component.cost,
    component.regionId
  );
  ...
  components.forEach((component) => {
    ...
    if (component.value !== null) {
      if (typeof component.value === "number")
        tempValue = `i;${component.value}`;
      else if (typeof component.value === "string")
        tempValue = `s;${component.value}`;
      else if (typeof component.value === "object")
        tempValue = `i;${component.value.resourceId}`;
    }
...
exports.getComponentResourceFunctionalByStateId =
  getComponentResourceFunctionalByStateId;
