<template>
  <div class="text-center w-full p-4">
    ...
    >
      <ul
        class="px-8 flex flex-row py-3 border-b-2 w-2/3 mx-auto justify-between"
      >
        <li class="">
          <router-link
    ...
    <router-view
      ...
      :state-facility-data="groupedStateFacilities"
      :state-region-data="stateRegions"
      :state-trade-agreements="stateTradeAgreements"
    ></router-view>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    groupedStateFacilities() {
      return this.groupFacilityByRegion(
        this.groupFacilitiesByCategory(
          this.$store.getters.getViewedStateFacilities,
        ),
      );
    },
    groupedStateResources() {
      return this.groupDataByRegion(
        this.$store.getters.getViewedStateResources,
      );
    },
    stateResourceCount() {
