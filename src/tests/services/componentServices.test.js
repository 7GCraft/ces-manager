...
describe('Modify Component Table', () => {
    ...
    describe('addComponent', () => {
        test('given just a component model should insert a component, decrease state treasury by its cost, and return OK', async () => {
            ...
            let resStatus = await componentServices.addComponent({ component: newComponent });

            let componentResult = await knex(constants.TABLE_COMPONENT).select('*');
            let facilityResult = await knex(constants.TABLE_FACILITY).select('*');
            let stateResult = (await knex(constants.TABLE_STATE).select('*'))[0];
        ...
        test('given a component model with temporary facility name should insert a component, insert a facility, decrease state treasury by its cost, and return OK', async () => {
            ...
            let addComponentParameter = {
                component: newComponent,
                tempFacilityName: tempFacilityName
            };
            let resStatus = await componentServices.addComponent(addComponentParameter);
    ...
    describe('updateComponent', () => {
        ...
        test('given just a component model should update the component and return OK', async () => {
            ...
            let resStatus = await componentServices.updateComponent({ component: updateComponent });
        ...
        test('given a component model with temporary facility name update the component, insert a facility, and return OK', async () => {
            ...
            let expectedComponent = JSON.parse(JSON.stringify(baseExpectedComponent));
            expectedComponent.facilityId = 1;

            const tempFacilityName = "New Facility";
            let updateComponentParameter = {
                component: updateComponent,
                tempFacilityName: tempFacilityName
            };
            let resStatus = await componentServices.updateComponent(updateComponentParameter);
    ...
    describe('addMultipleComponents', () => {
        ...
        test('given component models without temporary facility map should insert those components, decrease state treasury by total cost, and return OK', async () => {
            ...
            components.forEach(component => {
                const tempComponent = componentGeneralGenerator(
                    component.name, component.componentTypeId, component.cost, component.facilityId, component.value, component.activationTime
                );
            ...
            let resStatus = await componentServices.addMultipleComponents({ components: addComponents });
        ...
        test('given component models with temporary facility map should insert those components, decrease state treasury by total cost, insert the facilities, and return OK', async () => {
            ...
            components.forEach(component => {
                ...
                tempComponent.databaseModel.facilityId = 
                    tempComponent.databaseModel.facilityId == null ?
                    null :
            ...
            let addComponentsParameter = {
                components: addComponents,
                tempFacilityMap: facilityMap
            };

            let resStatus = await componentServices.addMultipleComponents(addComponentsParameter);
