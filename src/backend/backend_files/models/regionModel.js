...
module.exports = class Region {
      ...
      for (let facility of facilities) {
        if (facility.isFunctional) {
          if (facility.foodOutput > 0)
            this.totalFoodProduced += facility.foodOutput;
          else if (facility.foodOutput < 0)
            this.totalFoodConsumed -= facility.foodOutput;
  ...
  calculateGrowth(baseGrowth, regionCount) {
    if (baseGrowth > 0)
      this.expectedPopulationGrowth = Math.round(
        (baseGrowth / regionCount) * this.development.growthModifier,
      );
    else if (baseGrowth < 0)
      this.expectedPopulationGrowth = Math.round(
        (baseGrowth / regionCount) * this.development.shrinkageModifier,
      );
  }
};
