...
const advanceSeason = async () => {
  ...
  for (let state of states) {
    ...
    await stateServices.updateStateTreasuryByStateId(
      state.stateID,
      state.treasuryAmt + seasonalIncome,
    );
...
const populateStateInfo = async (initialState, updatedState, i) => {
  ...
  let facilityCount = await facility.getFacilityCountByStateId(
    updatedState[i].stateID,
  );
  let adminCost = await stateServices.getAdminCostByStateId(
    updatedState[i].stateID,
  );
  let updatedExpectedIncome = parseFloat(
    parseFloat(updatedState[i].TotalIncome).toFixed(2) -
      parseFloat(updatedState[i].expenses).toFixed(2) -
      parseFloat(adminCost).toFixed(2),
  ).toFixed(2);
...
const reformatStateResources = async (updatedState, i) => {
  ...
  updatedState[i].ProductiveResources.map((resource) => {
    resourceIdx = stateResources.findIndex(
      (obj) => obj.name == resource.ResourceName,
    );
    if (resourceIdx >= 0) {
...
const reformatTradeAgreements = async (tradeAgreements, initialState, i) => {
  ...
  tradeAgreements.map((agreement) => {
    let traderIdx = agreement.traders.findIndex(
      (obj) => obj.state.stateID == initialState[i].stateID,
    );
    if (traderIdx > -1) {
      stateTradeAgreements.push({
        partnerState:
          traderIdx == 0
            ? agreement.traders[1].state.stateName
            : agreement.traders[0].state.stateName,
        tradePower:
          parseFloat(agreement.traders[traderIdx].tradePower * 100).toFixed(2) +
          "%",
        tradeValue: agreement.traders[traderIdx].tradeValue,
      });
    }
  });
...
const formatCell = async (sheet, baseCell, targetCell, value, cellColor) => {
  ...
  sheet.getCell(baseCell).alignment = {
    vertical: "middle",
    horizontal: "center",
  };
...
const exportToExcel = async (
  ...
  try {
    ...
    for (let i = 0; i < initialState.length; i++) {
      ...
      await formatCell(sheet, "A1", "L3", initialState[i].stateName, [
        RED,
        ORANGE,
        RED,
      ]);
      ...
      await formatCell(sheet, "A6", "D6", "General State Info", [
        RED,
        ORANGE,
        RED,
      ]);
      ...
      let increment = 0;
      sheet.getCell("A" + (7 + increment).toString()).value =
        stateInfoRowNames[0];
      sheet.getCell("B" + (7 + increment).toString()).value =
        initialStateInfo[0];
      sheet.getCell("C" + (7 + increment).toString()).value = "to";
      sheet.getCell("D" + (7 + increment).toString()).value =
        updatedStateInfo[0];
      increment += 1;
      ...
      for (let j = 0; j < updatedState[i].regions.length; j++) {
        sheet.getCell("G" + (8 + increment).toString()).value =
          updatedState[i].regions[j].regionName;
        sheet.getCell("H" + (8 + increment).toString()).value =
          updatedState[i].regions[j].totalIncome;
        sheet.getCell("I" + (8 + increment).toString()).value =
          updatedState[i].regions[j].totalFoodAvailable;
        sheet.getCell("J" + (8 + increment).toString()).value =
          updatedState[i].regions[j].usedPopulation;
        sheet.getCell("K" + (8 + increment).toString()).value =
          initialState[i].regions[j].population +
          " -> " +
          updatedState[i].regions[j].population;
        sheet.getCell("L" + (8 + increment).toString()).value =
          getPopulationCap(
            updatedState[i].regions[j].development.developmentId,
          );
        increment += 1;
      }
      ...
      if (Array.isArray(stateResources) && stateResources.length) {
        ...
        sheet.getCell("C" + (lastRowNum + increment).toString()).value =
          "Count";
        ...
        stateResources.forEach((resource) => {
          sheet.getCell("A" + (lastRowNum + increment).toString()).value =
            resource.name;
          sheet.getCell("B" + (lastRowNum + increment).toString()).value =
            resource.tier;
          sheet.getCell("C" + (lastRowNum + increment).toString()).value =
            resource.count;
          increment += 1;
      ...
      let stateTradeAgreements = await reformatTradeAgreements(
        tradeAgreements,
        initialState,
        i,
      );
      ...
      if (Array.isArray(stateTradeAgreements) && stateTradeAgreements.length) {
        ...
        sheet.getCell("G" + (lastRowNum + increment).toString()).value =
          "Trade Partner";
        sheet.getCell("H" + (lastRowNum + increment).toString()).value =
          "Our Trade Power";
        sheet.getCell("I" + (lastRowNum + increment).toString()).value =
          "Income From Trade";
        ...
        stateTradeAgreements.forEach((agreement) => {
          sheet.getCell("G" + (lastRowNum + increment).toString()).value =
            agreement.partnerState;
          sheet.getCell("H" + (lastRowNum + increment).toString()).value =
            agreement.tradePower;
          sheet.getCell("I" + (lastRowNum + increment).toString()).value =
            agreement.tradeValue;
          increment += 1;
        });
      }
    }
