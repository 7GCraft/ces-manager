...
const getRegionListAll = async () => {
  ...
  for (let rawRegion of rawRegions) {
    let facilities = await facilityServices.getFacilitiesByRegionId(
      rawRegion.regionId,
    );
...
const getRegionListByStateId = async (stateId) => {
  ...
  let regionsWithUsedPopulations = await knex
    ...
    .where(constants.TABLE_REGION + "." + constants.COLUMN_STATE_ID, stateId)
    .where(
      constants.TABLE_COMPONENT + "." + constants.COLUMN_COMPONENT_TYPE_ID,
      1,
    )
    .where(
      constants.TABLE_COMPONENT + "." + constants.COLUMN_ACTIVATION_TIME,
      0,
    )
    .catch((e) => {
      console.error(e);
  ...
  for (let rawRegion of rawRegions) {
    ...
    if (
      regionsWithUsedPopulationsDict != null &&
      regionsWithUsedPopulationsDict[rawRegion.regionId] != undefined
    ) {
      rawUsedPopulations = regionsWithUsedPopulationsDict[
        rawRegion.regionId
      ].map((val) => {
        return parseInt(val.split(";")[1]);
      });
      usedPopulation = rawUsedPopulations.reduce((total, num) => {
        return total + num;
      });
    }
...
const getRegionByStateId = async (id) => {
  let rawRegions = await knex
    .select([
      ...
      constants.TABLE_STATE +
        "." +
        constants.COLUMN_EXPENSES +
        " AS stateExpenses",
      constants.TABLE_CORRUPTION +
        "." +
        constants.COLUMN_CORRUPTION_ID +
        " AS corruptionId",
      constants.TABLE_CORRUPTION +
        "." +
...
const getDevelopmentAll = async () => {
  ...
  for (let rawDevelopment of rawDevelopments) {
    let development = new Development(
      rawDevelopment.developmentId,
      rawDevelopment.name,
      rawDevelopment.populationCap,
      rawDevelopment.militaryTier,
      rawDevelopment.growthModifier,
      rawDevelopment.shrinkageModifier,
    );
