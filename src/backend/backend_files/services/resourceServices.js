...
const getResourceTierById = async function (id) {
  ...
  let resourceTier = new ResourceTier(
    rawResourceTier.resourceTierId,
    rawResourceTier.name,
    rawResourceTier.tradePower,
    [],
  );
...
const getAllResourcesByStateId = async function (stateId) {
  let resourceIdWithFunctionals = await knex
    ...
    .select(
      constants.COLUMN_COMPONENT_VALUE,
  ...
  let resources = await knex
    ...
    .from(constants.TABLE_RESOURCE)
    .whereIn(
      constants.COLUMN_RESOURCE_ID,
      Object.keys(resourceWithProductiveCountDict),
    )
    .catch((e) => {
      console.error(e);
    });
  ...
  for (let resource of resources) {
    ...
    results.push({
      ...
      CountProductive:
        resourceWithProductiveCountDict[resource.resourceId].countProductive,
    });
  }
  return results;
};
