...
const FUNCTION_TOKENS = ["MIN", "MAX", "ROUND"];
...
function parseGroupExpression(currentToken, parseStack, resultStack) {
  if (currentToken == TOKEN.OPEN_PARENTHESIS) {
    ...
    for (
      let char = parseStack.pop();
      char != TOKEN.OPEN_PARENTHESIS;
      char = parseStack.pop()
    ) {
      if (char === undefined) {
...
function isRedundantToken(char) {
  const redundantTokens = [TOKEN.SPACE, TOKEN.COMMA];
...
function evaluate(evalStack, vars) {
  ...
  for (let i = 0; i < evalStack.length; i++) {
    ...
    let processArgs = [tokenObj.token, resultStack];
